...
class ThreeDConv(nn.Module):
    def __init__(self,in_planes,planes,stride=1):
        ...
        # Pointwise conv
        self.conv2 = nn.Conv3d(in_planes, planes, kernel_size=1, stride=1, padding=0, bias=False)
        self.bn2 = nn.BatchNorm3d(planes)

        # Depthwise conv
        self.conv3 = nn.Conv3d(planes, planes, kernel_size=3, stride=1, padding=1, groups=planes,
                               bias=False)
        self.bn3 = nn.BatchNorm3d(planes)

        # Pointwise conv
        self.conv4 = nn.Conv3d(planes, planes, kernel_size=1, stride=1, padding=0, bias=False)
        self.bn4 = nn.BatchNorm3d(planes)

        # Depthwise conv
        self.conv5 = nn.Conv3d(planes, planes, kernel_size=3, stride=1, padding=1, groups=planes,
                               bias=False)
        self.bn5 = nn.BatchNorm3d(planes)

        # Pointwise conv
        self.conv6 = nn.Conv3d(planes, planes, kernel_size=1, stride=1, padding=0, bias=False)
...
class GC_NET(nn.Module):
    def __init__(self,block,block_3d,num_block,height,width,maxdisp,batch):
        ...
        # Depthwise conv
        self.conv3d_1 = nn.Conv3d(64, 64, kernel_size=3, stride=1, padding=1, groups=64,
                               bias=False)
        self.bn3d_1 = nn.BatchNorm3d(64)

        # Pointwise conv
        self.conv3d_2 = nn.Conv3d(64, 32, kernel_size=1, stride=1, padding=0, bias=False)
        ...
        # Depthwise conv
        self.conv3d_3 = nn.Conv3d(32, 32, kernel_size=3, stride=1, padding=1, groups=32,
                                  bias=False)
        self.bn3d_3 = nn.BatchNorm3d(32)

        # Pointwise conv
        self.conv3d_4 = nn.Conv3d(32, 32, kernel_size=1, stride=1, padding=0, bias=False)
        self.bn3d_4 = nn.BatchNorm3d(32)

        # self.conv3d_3=nn.Conv3d(64,64,3,2,1)
        # self.bn3d_3=nn.BatchNorm3d(64)

        # Depthwise conv
        self.conv3d_5 = nn.Conv3d(64, 64, kernel_size=3, stride=2, padding=1, groups=64,
                                  bias=False)
        self.bn3d_5 = nn.BatchNorm3d(64)

        # Pointwise conv
        self.conv3d_6 = nn.Conv3d(64, 64, kernel_size=1, stride=1, padding=0, bias=False)
        self.bn3d_6 = nn.BatchNorm3d(64)


        # self.conv3d_4=nn.Conv3d(64,64,3,2,1)
        # self.bn3d_4=nn.BatchNorm3d(64)
        # Depthwise conv
        self.conv3d_7 = nn.Conv3d(64, 64, kernel_size=3, stride=2, padding=1, groups=64,
                                  bias=False)
        self.bn3d_7 = nn.BatchNorm3d(64)

        # Pointwise conv
        self.conv3d_8 = nn.Conv3d(64, 64, kernel_size=1, stride=1, padding=0, bias=False)
        self.bn3d_8 = nn.BatchNorm3d(64)

        # self.conv3d_5=nn.Conv3d(64,64,3,2,1)
        # self.bn3d_5=nn.BatchNorm3d(64)
        # Depthwise conv
        self.conv3d_9 = nn.Conv3d(64, 64, kernel_size=3, stride=2, padding=1, groups=64,
                                  bias=False)
        self.bn3d_9 = nn.BatchNorm3d(64)

        # Pointwise conv
        self.conv3d_10 = nn.Conv3d(64, 64, kernel_size=1, stride=1, padding=0, bias=False)
        self.bn3d_10 = nn.BatchNorm3d(64)
        ...
        self.deconv1 = nn.ConvTranspose3d(128, 128, 3, 2, 1, 1, 128)
        self.debn1 = nn.BatchNorm3d(128)
        self.deconv2 = nn.ConvTranspose3d(128, 64, 1, 1, 0, 0)
        ...
        self.debn5 = nn.BatchNorm3d(64)
        self.deconv6 = nn.ConvTranspose3d(64, 64, 1, 1, 0, 0)
        self.debn6 = nn.BatchNorm3d(64)

        self.deconv7 = nn.ConvTranspose3d(64, 64, 3, 2, 1, 1, 64)
        self.debn7 = nn.BatchNorm3d(64)
        self.deconv8 = nn.ConvTranspose3d(64, 32, 1, 1, 0, 0)
        self.debn8 = nn.BatchNorm3d(32)

        self.deconv9 = nn.ConvTranspose3d(32, 32, 3, 2, 1, 1, 32)
        self.debn9 = nn.BatchNorm3d(32)
        self.deconv10 = nn.ConvTranspose3d(32, 1, 1, 1, 0, 0)
    ...
    def forward(self, imgLeft,imgRight):
        ...
        # cost volume
        cost_volum = self.cost_volume(imgl1,imgr1, self.batch)
        ...
        # conv3d_out=F.relu(self.bn3d_2(self.conv3d_2(conv3d_out)))
        conv3d_out = F.relu(self.bn3d_3(self.conv3d_3(conv3d_out)))
        conv3d_out = F.relu(self.bn3d_4(self.conv3d_4(conv3d_out)))

        #conv3d block
        conv3d_block_1=self.block_3d_1(cost_volum)
        # conv3d_21=F.relu(self.bn3d_3(self.conv3d_3(cost_volum)))
        conv3d_21 = F.relu(self.bn3d_5(self.conv3d_5(cost_volum)))
        conv3d_21 = F.relu(self.bn3d_6(self.conv3d_6(conv3d_21)))

        conv3d_block_2=self.block_3d_2(conv3d_21)
        # conv3d_24=F.relu(self.bn3d_4(self.conv3d_4(conv3d_21)))
        conv3d_24 = F.relu(self.bn3d_7(self.conv3d_7(conv3d_21)))
        conv3d_24 = F.relu(self.bn3d_8(self.conv3d_8(conv3d_24)))

        conv3d_block_3=self.block_3d_3(conv3d_24)
        # conv3d_27=F.relu(self.bn3d_5(self.conv3d_5(conv3d_24)))
        conv3d_27 = F.relu(self.bn3d_9(self.conv3d_9(conv3d_24)))
        conv3d_27 = F.relu(self.bn3d_10(self.conv3d_10(conv3d_27)))
        ...
        deconv3d = F.relu(self.debn6(self.deconv6(deconv3d)) + conv3d_block_1)
        deconv3d = F.relu(self.debn7(self.deconv7(deconv3d)))
        deconv3d = F.relu(self.debn8(self.deconv8(deconv3d)) + conv3d_out)



        #last deconv3d
        # deconv3d=self.deconv5(deconv3d)
        deconv3d = F.relu(self.debn9(self.deconv9(deconv3d)))
        deconv3d = self.deconv10(deconv3d)
    ...
    def cost_volume(self,imgl,imgr, batch):
        xx_list = []
        ...
        xx = xx.view(batch, self.maxdisp, 64, int(self.height / 2), int(self.width / 2) + self.maxdisp)
