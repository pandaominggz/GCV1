...
def train(epoch_total,loadstate):
    ...
    if loadstate==True:
        ...
        # checkpoint = torch.load('/home/s1804054/GVV1/checkpoint/ckpt.t7')
        checkpoint = torch.load('/home/ubuntu/GVV1/checkpoint/ckpt.t7')
    ...
    for epoch in range(start_epoch,epoch_total):
        ...
        for step in range(len(dataloader)):
            ...
            if step==len(dataloader)-1:
                ...
                state = {'net': net.state_dict(), 'step': step, 'epoch': epoch, 'accur': acc_total}
                torch.save(state,'/home/ubuntu/GVV1/checkpoint/ckpt.t7')
                for j in range(batch):
                    im = result[j, :, :, :].data.cpu().numpy().astype('uint8')
                    im = np.transpose(im, (1, 2, 0))
                    a = name[j].split('/')[-1].split('.')[0]
                    cv2.imwrite('/home/ubuntu/GVV1/train_result_'+a+'.png', im, [int(cv2.IMWRITE_PNG_COMPRESSION), 0])
                    gt = np.transpose(dispL[j, :, :, :].data.cpu().numpy(), (1, 2, 0))
                    cv2.imwrite('/home/ubuntu/GVV1/train_gt_'+a+'.png', gt, [int(cv2.IMWRITE_PNG_COMPRESSION), 0])

    fp=open('/home/ubuntu/GVV1/loss.txt','w')
    fa= open('/home/ubuntu/GVV1/acc.txt','w')
    for i in range(len(loss_list)):
        fp.write(str(loss_list[i]))
        fp.write('\n')
        fa.write(str(acc_list[i]))
        fa.write('\n')
    fp.close()
    fa.close()
